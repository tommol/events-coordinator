...
public sealed class PublishOccasionCommandHandler(
    IAggregateRepository<Occasion, OccasionId> repository,
    ILogger<PublishOccasionCommandHandler> logger)
    : ICommandHandler<PublishOccasionCommand>
{
    public async Task Handle(PublishOccasionCommand command, CancellationToken cancellationToken = default)
    {
        try
        {
