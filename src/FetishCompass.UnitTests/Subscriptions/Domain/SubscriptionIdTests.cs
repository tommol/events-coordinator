using FetishCompass.Domain.Subscriptions.Model;

namespace FetishCompass.UnitTests.Subscriptions.Domain;
 ...
 public class SubscriptionIdTests
    {
        ...
        {
            var id1 = SubscriptionId.New();
            var id2 = SubscriptionId.New();
        ...
        public void ToString_ReturnsGuidString()
        {
            ...
            var id = new SubscriptionId(guid);
        ...
        {
            var guid = Guid.NewGuid();
            SubscriptionId id = (SubscriptionId)guid;
            Guid guid2 = id;
        ...
        public void Parse_StringAndSpan_Works()
        {
            ...
            var id1 = SubscriptionId.Parse(str);
            var id2 = SubscriptionId.Parse(str.AsSpan(), null);
        ...
        public void TryParse_StringAndSpan_ValidAndInvalid()
        {
            var guid = Guid.NewGuid();
            var str = guid.ToString();
            Assert.True(SubscriptionId.TryParse(str, null, out var id1));
            Assert.Equal(guid, id1.Value);
            Assert.True(SubscriptionId.TryParse(str.AsSpan(), null, out var id2));
            Assert.Equal(guid, id2.Value);
            Assert.False(SubscriptionId.TryParse("not-a-guid", null, out _));
            Assert.False(SubscriptionId.TryParse("not-a-guid".AsSpan(), null, out _));
        }
