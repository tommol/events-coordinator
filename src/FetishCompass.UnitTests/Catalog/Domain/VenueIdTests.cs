 ...
 public class VenueIdTests
    {
        ...
        public void Parse_StringAndSpan_Works()
        {
            ...
            var id1 = VenueId.Parse(str);
            var id2 = VenueId.Parse(str.AsSpan(), null);
        ...
        public void TryParse_StringAndSpan_ValidAndInvalid()
        {
            var guid = Guid.NewGuid();
            var str = guid.ToString();
            Assert.True(VenueId.TryParse(str, null, out var id1));
            Assert.Equal(guid, id1.Value);
            Assert.True(VenueId.TryParse(str.AsSpan(), null, out var id2));
            Assert.Equal(guid, id2.Value);
            Assert.False(VenueId.TryParse("not-a-guid", null, out _));
            Assert.False(VenueId.TryParse("not-a-guid".AsSpan(), null, out _));
        }
