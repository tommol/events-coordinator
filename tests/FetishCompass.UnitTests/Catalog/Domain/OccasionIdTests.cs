...
{
    public class OccasionIdTests
    {
        [Fact]
        public void New_GeneratesUniqueGuid()
        {
            var id1 = OccasionId.New();
            var id2 = OccasionId.New();
        ...
        public void Parse_StringAndSpan_Works()
        {
            ...
            var id1 = OccasionId.Parse(str);
            var id2 = OccasionId.Parse(str.AsSpan(), null);
        ...
        public void TryParse_StringAndSpan_ValidAndInvalid()
        {
            var guid = Guid.NewGuid();
            var str = guid.ToString();
            Assert.True(OccasionId.TryParse(str, null, out var id1));
            Assert.Equal(guid, id1.Value);
            Assert.True(OccasionId.TryParse(str.AsSpan(), null, out var id2));
            Assert.Equal(guid, id2.Value);
            Assert.False(OccasionId.TryParse("not-a-guid", null, out _));
            Assert.False(OccasionId.TryParse("not-a-guid".AsSpan(), null, out _));
        }
